...
> [!TIP]
> By default it uses a tiny model (d_model=256, 4 layers, 4 heads) that trains on 5,000 Wikipedia articles for 500 steps. This takes about 2-5 minutes on CPU or a few seconds on GPU.

```bash
python main.py
```

The script also contains a commented-out GPT-2 small configuration (768 dims, 12 layers, 12 heads) for when you have a GPU.
