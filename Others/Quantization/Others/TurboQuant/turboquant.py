# An unofficial, end-to-end PyTorch implementation of TurboQuant
#
# Reference: arXiv:2504.19874
# "TurboQuant: Online Vector Quantization with Near-optimal Distortion Rate"
#
# Two-stage key quantizer:
#   Qmse  — Haar random rotation + per-coordinate Lloyd-Max scalar quantization.
#   Qprod — 1-bit QJL residual sketch on top of Qmse for unbiased dot-product estimation.
#
# Values are compressed with Qmse only.
# Tested against Qwen/Qwen2.5-3B-Instruct (dense, non-MoE).
...
from transformers.models.qwen2.modeling_qwen2 import (
    ALL_ATTENTION_FUNCTIONS as attention_dispatch,
    Qwen2Attention,
...
def build_haar_rotation(d: int, seed: Optional[int] = None, device: str = "cpu") -> torch.Tensor:
...
def build_qjl_projection(
    d: int,
    m: Optional[int] = None,
    seed: Optional[int] = None,
    device: str = "cpu",
) -> torch.Tensor:
    """Random Gaussian projection matrix for the QJL sketch."""
    if m is None:
        m = d
    gen = torch.Generator(device="cpu")
    if seed is not None:
...
class TurboQuantMSECompressor:
    def __init__(self, head_dim: int, bits: int, seed: int, device: str = "cpu"):
        ...
        self.bits = bits
        self.device = device
        self.rotation_matrix = build_haar_rotation(head_dim, seed=seed, device=device)
    ...
    def compress(self, states: torch.Tensor) -> dict:
        ...
        rotated = flat_norm @ self.rotation_matrix.T
    ...
    def reconstruct(self, compressed: dict) -> torch.Tensor:
        ...
        return (self.centroids[indices] @ self.rotation_matrix) * vec_norms
...
class TurboQuantProdCompressor:
    def __init__(self, head_dim: int, bits: int, seed: int, device: str = "cpu"):
        ...
        self.qjl_projection = build_qjl_projection(head_dim, head_dim, seed=seed + 10_000, device=device)
        self.qjl_dim = self.qjl_projection.shape[0]
    ...
    def compress(self, states: torch.Tensor) -> dict:
        ...
        projected = residual @ self.qjl_projection.T
    ...
    def compute_attention_scores(
        ...
        *,
        ...
        for compressed_k in self._compressed_keys:
            ...
            q_projected = torch.matmul(query_states_grouped.float(), self._key_compressor.qjl_projection.T)
    ...
    **kwargs,
    ...
    if isinstance(past_key_values, (TurboQuantPaperCache, TurboQuantGenerationCache)):
        ...
        attention_interface = (
            ...
            else attention_dispatch[self.config._attn_implementation]
        )
...
def patch_attention(model) -> None:
    """Replace Qwen2Attention.forward with the TurboQuant-aware variant"""
...
def unpatch_attention(model) -> None:
    """Restore original Qwen2Attention.forward on all layers"""
    for module in model.modules():
        if isinstance(module, Qwen2Attention) and hasattr(module, "_turboquant_original_forward"):
...
if __name__ == "__main__":
    ...
    # print("TurboQuant Qwen2 attention patch applied\n")

    # Few-shot extraction prompt: domain-relevant (LLM architecture) so the task
    # exercises the model's parametric knowledge under quantized KV cache.
    system_prompt = "You are a helpful assistant. Answer factual questions precisely."
    user_prompt = (
        "Context: The Transformer architecture introduced by Vaswani et al. uses "
        "multi-head self-attention with a mechanism called Scaled Dot-Product Attention.\n"
        "Question: What attention mechanism does the Transformer use?\n"
        "Answer: Scaled Dot-Product Attention\n\n"
        "Context: Grouped Query Attention (GQA) reduces KV memory by sharing key-value "
        "heads across multiple query heads, first adopted in LLaMA-2 70B.\n"
        "Question: What model first adopted GQA?\n"
        "Answer: LLaMA-2 70B\n\n"
        "Context: Qwen2.5-3B-Instruct is a 3.09B parameter causal language model with "
        "36 transformer layers, using RoPE and SwiGLU.\n"
        "Question: How many transformer layers does Qwen2.5-3B-Instruct have?\n"
        "Answer:"
    )
    ...
    with torch.no_grad():
        ...
        results["FP16 Baseline"] = ans
        print(f"FP16 Baseline: {ans}")

    # TurboQuant 3-bit (Qmse + Qprod)
    tq_cache_3 = TurboQuantPaperCache.from_model_config(model.config, bits=3)
    with torch.no_grad():
        out = model.generate(
            **inputs,
            past_key_values=tq_cache_3,
            max_new_tokens=5,
            do_sample=False,
            pad_token_id=tokenizer.eos_token_id,
        )
        ans = tokenizer.decode(out[0][inputs["input_ids"].shape[1]:], skip_special_tokens=True).strip()
        results["TQ 3-bit"] = ans
        print(f"TQ 3-bit     : {ans}")

    # TurboQuant 4-bit (Qmse + Qprod)
    ...
    with torch.no_grad():
        ...
        ans = tokenizer.decode(out[0][inputs["input_ids"].shape[1]:], skip_special_tokens=True).strip()
        results["TQ 4-bit"] = ans
        print(f"TQ 4-bit     : {ans}")

    # MSE-only 4-bit (no residual sketch)
    ...
    with torch.no_grad():
        ...
        ans = tokenizer.decode(out[0][inputs["input_ids"].shape[1]:], skip_special_tokens=True).strip()
        results["MSE 4-bit"] = ans
        print(f"MSE 4-bit    : {ans}")

    print("\nEval: Exact match comparison against FP16 baseline")

    baseline = results["FP16 Baseline"]

    for label, output in results.items():
        if label == "FP16 Baseline":
            continue
        match = "MATCH" if output == baseline else "MISMATCH"
        print(f"{label:<15} | {match:<8} | '{output}'")

    print("-" * 64 + "\n")

    layer_stats = tq_cache_3.layers[0].stats
    print("Cache Layer 0 Compression:")
    print(f"  FP16 equivalent : {layer_stats.fp16_visible_bytes / 1024:.2f} KB")
    print(f"  TQ 3-bit actual : {layer_stats.compressed_bytes / 1024:.2f} KB")